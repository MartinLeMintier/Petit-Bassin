...
session_start();
		...
		$requete = $bdd->prepare('SELECT ID FROM amis WHERE IDami= ? AND ID= ?');
		$requete->execute(array($IDami,$monID));
		
		$lami = $requete->fetch();
		
		$requete2 = $bdd->prepare('SELECT IDami FROM amis WHERE ID= ? AND IDami=?');
		...
		$lami2 = $requete2->fetch();
				...
				{	echo 'machin';
					...
					$monami = $bdd->prepare('INSERT INTO amis(ID, IDami) VALUES (:ID,:IDami)');
					...
					header('Location: demandeami.php');	
				}
